...
namespace TNAUPMS.WebApi.Controllers.Master
{
    ...
    [Route("api/sciencemeet")]
    public class ScienceMeetController : TNAUPMSApiController<massciencemeet, ImassciencemeetRepository>
    {
        private readonly IHostingEnvironment m_hostingEnvironment;
        public ScienceMeetController(IHostingEnvironment p_hostingEnvironment)
        {
            m_hostingEnvironment = p_hostingEnvironment;
        }
        [HttpGet("get-sciencemeetbyid/{p_Id}")]
        public async Task<massciencemeet> GetScienceAsync(string p_Id)
        {
            if (!string.IsNullOrEmpty(this.SecurityContext.GetUsername()))
            {
                int fId = int.Parse(p_Id);
                massciencemeet inst = await this.Repository.GetByIdAsync(fId);
                return inst;
            }
            return null;
        }
        ...
        [HttpGet("get-sciencemeet-all")]
        public async Task<List<massciencemeet>> GetScienceAllAsync()
        {
            if (!string.IsNullOrEmpty(this.SecurityContext.GetUsername()))
            {
                List<massciencemeet> inst = await this.Repository.GetAllScienceMeet();
                return inst;
            }
            return null;
        }

        [HttpPost("save-sciencemeet")]
        public async Task<AuthResult> SaveScienceMeet([FromBody] massciencemeet p_science)
        {
        ...
        [HttpPost("remove/{p_SMId}")]
        public async Task<AuthResult> RemoveCategory(string p_SMId)
        {
            int _id = int.Parse(p_SMId);
            ...
            try
            {
                ...
                {
                    int isactive = 0;
                    massciencemeet scinence = await this.Repository.GetByIdAsync(_id);
        ...
        private async Task<List<string>> IsValid(massciencemeet p_fund, bool isNew = true)
        {
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(p_fund.ScienceMeetName))
                    ErrorMsgs.Add("Science Meet Name is required");
            }
